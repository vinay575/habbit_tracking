...
const SidebarProvider = React.forwardRef(({ defaultOpen = true, open, onOpenChange, className, style, children, ...props }, ref) => {
  ...
  // We use openMobile and setOpenMobile for mobile sidebar state.
  const [_open, _setOpen] = React.useState(defaultOpen)
  const openState = open ?? _open
  const setOpenState = React.useCallback(
    (value) => {
      const openState = typeof value === "function" ? value(open ?? _open) : value
      if (onOpenChange) {
...
const SidebarMenuSkeleton = React.forwardRef(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="h-4 max-w-[--skeleton-width] flex-1 rounded-sm bg-sidebar-accent-foreground/20"
        style={{
